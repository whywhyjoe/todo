...
function hoverBackground(color) {
 ...
 const shifted = `color-mix(in srgb, ${color} ${100 - state.hoverShift}%, white)`;
 return `color-mix(in srgb, ${shifted} ${state.textBgOpacity}, transparent)`;
}
...
$('toggle-code').addEventListener('click', event => {
 ...
 event.currentTarget.textContent = showing ? 'Show Code' : 'Hide Code';
...
stageResizer.addEventListener('pointerdown', event => {
 ...
 const maxWidth = stageInner.parentElement.clientWidth;
